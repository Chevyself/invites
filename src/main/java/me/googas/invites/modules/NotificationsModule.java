...
public class NotificationsModule implements Module {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        TeamMember member = Invites.getLoader().getSubloader(MembersSubloader.class).getMember(event.getPlayer());
        Invites.getLoader().getSubloader(InvitationsSubloader.class).getInvitations(member, InvitationStatus.WAITING).forEach(invitation -> {
            String name = invitation.getLeader().getTeam().map(Team::getName).orElse("deleted");
            member.localized("invitations.invite.received", MapBuilder.of("team", name).put("member", invitation.getLeader().getName()).build());
        });
    }
