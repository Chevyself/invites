...
public class SqlMembersSubloader extends LazySQLSubloader implements MembersSubloader {
  ...
  public @NonNull List<SqlTeamMember> getMembers(@NonNull Team team) {
    return this.statement("SELECT * FROM `members` WHERE `team`=?;")
        .execute(
            statement -> {
  ...
  public boolean setTeam(@NonNull SqlTeamMember member, Team team, TeamRole role) {
    return this.statement("UPDATE `members` SET `team`=?, `role`=? WHERE `uuid`=?;")
        .execute(
            statement -> {
              if (team == null) {
  ...
  public SqlTeamMember getMember(@NonNull OfflinePlayer player) {
    SqlTeamMember sqlMember =
        this.parent
            ...
            .orElseGet(
                () ->
                    this.statement("SELECT * FROM `members` WHERE `uuid`=? LIMIT 1;")
                        .execute(
                            statement -> {
                              ...
                              if (query.next()) {
                                ...
                                return this.statement("INSERT INTO `members`(`uuid`) VALUES(?);")
                                    .execute(
                                        insert -> {
                                          insert.setString(1, player.getUniqueId().toString());
                                          insert.execute();
                                          SqlTeamMember member =
                                              new SqlTeamMember(player.getUniqueId(), -1, null);
  ...
  public @NonNull List<SqlTeamMember> getLeaders() {
    return this.statement("SELECT * FROM `members` WHERE `role`=?;")
        .execute(
            statement -> {
