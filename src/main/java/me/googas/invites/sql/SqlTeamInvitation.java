...
public class SqlTeamInvitation implements TeamInvitation, SQLElement, Catchable {
    ...
    public SqlTeamInvitation(int id, @NonNull UUID invited, @NonNull UUID leader, @NonNull InvitationStatus status) {
        this.id = id;
    ...
    public static SqlTeamInvitation of(@NonNull ResultSet resultSet) throws SQLException {
        return new SqlTeamInvitation(resultSet.getInt("id"), UUID.fromString(resultSet.getString("invited")), UUID.fromString(resultSet.getString("leader")), InvitationStatus.valueOf(resultSet.getString("status")));
    }
    ...
    public @NonNull TeamMember getLeader() {
        return Invites.getLoader().getSubloader(SqlMembersSubloader.class).getMember(Bukkit.getOfflinePlayer(this.leader));
    }

    @Override
    public @NonNull TeamMember getInvited() {
        return Invites.getLoader().getSubloader(SqlMembersSubloader.class).getMember(Bukkit.getOfflinePlayer(this.invited));
    }
}
