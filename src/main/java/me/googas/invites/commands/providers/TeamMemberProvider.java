...
public class TeamMemberProvider implements BukkitExtraArgumentProvider<TeamMember>, BukkitArgumentProvider<TeamMember> {
    @Override
    public @NonNull TeamMember fromString(@NonNull String string, @NonNull CommandContext context) throws ArgumentProviderException {
        return Invites.getLoader().getSubloader(MembersSubloader.class).getMember(context.get(string, OfflinePlayer.class, context));
    }

    @Override
    public @NonNull List<String> getSuggestions(@NonNull String string, CommandContext commandContext) {
